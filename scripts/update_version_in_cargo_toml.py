...
def get_chrome_ext_compat_version() -> str:
    cfg = get_config()
    cfg.verbose = True
...
if __name__ == "__main__":
    manifest_path = Path(__file__).parent.parent / "rust" / "Cargo.toml"
    ...
    if len(sys.argv) > 1:
        ...
        print(f"Version in {manifest_path} has been set to {new_version}")
    else:
        if manifest["package"]["version"] == "0.0.0":
            ...
            print(
                f"Version in {manifest_path} is already set to {manifest['package']['version']}"
            )
